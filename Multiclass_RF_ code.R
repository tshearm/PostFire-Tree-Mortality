...
ABCO.cc <- select(ABCO, OBS_ID, CVS_percent, DBH_cm, BCH_percent, dead_check) %>%
  filter(dead_check!=9) %>% na.omit
...
ABGR.cc <- select(ABGR, OBS_ID, CVS_percent, DBH_cm, BCH_percent, dead_check) %>%
  filter(dead_check!=9) %>% na.omit
...
CADE27.cc <- select(CADE27, OBS_ID, CVS_percent, DBH_cm, BCH_percent, dead_check) %>%
  filter(dead_check!=9) %>% na.omit
...
LAOC.cc <- select(LAOC, OBS_ID, CVS_percent, DBH_cm, BCH_percent, dead_check) %>%
  filter(dead_check!=9) %>% na.omit
...
#######PICO##############
PICO.cc <- select(PICO, OBS_ID, CVS_percent, DBH_cm, BCH_percent, dead_check) %>%
  filter(dead_check!=9) %>% na.omit
...
PILA.cc <- select(PILA, OBS_ID, CVS_percent, DBH_cm, BCH_percent, dead_check) %>%
  filter(dead_check!=9) %>% na.omit
...
PIPO.cc <- select(PIPO, OBS_ID, CVS_percent, DBH_cm, BCH_percent, dead_check) %>%
  filter(dead_check!=9) %>% na.omit
...
PSME.cc <- select(PSME, OBS_ID, CVS_percent, DBH_cm, BCH_percent, dead_check) %>%
  filter(dead_check!=9) %>% na.omit
...
ABCO.cc.sum$Species <- "ABCO"
ABGR.cc.sum$Species <- "ABGR"
CADE27.cc.sum$Species <- "CADE27"
LAOC.cc.sum$Species <- "LAOC"
PICO.cc.sum$Species <- "PICO"
PILA.cc.sum$Species <- "PILA"
PIPO.cc.sum$Species <- "PIPO"
PSME.cc.sum$Species <- "PSME"

when_dead_acc <- rbind(ABCO.cc.sum, ABGR.cc.sum, CADE27.cc.sum, LAOC.cc.sum,
                       PICO.cc.sum, PILA.cc.sum, PIPO.cc.sum, PSME.cc.sum)
write.csv(when_dead_acc, "when_dead_acc.csv")
when_dead_acc %>%
  group_by(Status, Species) %>%
  summarise(meanC = mean(correct)) -> acc_per_spp

acc_per_spp %>%
  ungroup() %>%
  group_by(Status) %>%
  summarise(maxC = max(meanC),
            minC = min(meanC)) -> avg_acc_range
